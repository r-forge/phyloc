...
read.nexus <- function(file, tree.names = NULL)
{
    ...
    for (i in 1:nb.tree) {
        ...
        ## translation only if there is one tree:
        if (translation && nb.tree == 1) {
    ...
    if (nb.tree == 1) trees <- trees[[1]] else {
        ...
        if (translation) {
            n <- Ntip(trees[[1]])
            for (i in 1:nb.tree) {
                TIPS <- trees[[i]]$edge[, 2] <= n
                trees[[i]]$edge[TIPS, 2] <- as.numeric(trees[[i]]$tip.label)
                trees[[i]]$tip.label <- NULL
            }
            attr(trees, "Labels") <- TRANS[, 2]
        }
    }
