...
vectors, matrices, dates, and \R\ data frames between \R\ and C++.
\end{abstract}
...
matrices, Dates, and \R-style data frames (similar to data base tables).
...
({\tt RcppParams}),
\item A homogeneous list of named values of numeric type ({\tt RcppNamedList})
